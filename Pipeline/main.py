...
for num_experiment in range(1, 5): # всего 4 эксперимента
    ...
    best_score = train.train(model, optimizer, train_dataloader, val_dataloader, criterion=conf.cr, n_epochs=conf.def_n_ep, device=conf.dev)
    ...
    res.results[num_experiment] = best_score

# Итоговые результаты
with open(f'{conf.path_unzip}/results.pickle', 'wb') as f:
    pickle.dump(res.results, f)
